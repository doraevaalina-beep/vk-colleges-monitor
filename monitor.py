...
def wall_params(slug):
    ...
    params = {"v": API_VERSION, "count": POSTS_PER_SOURCE, "filter": "owner", "extended": 1}
    if m:
    ...
    for a in attachments or []:
        ...
        if t == "photo":
            ...
            if oid is not None and vid is not None:
                out.append({"type": "video", "title": obj.get("title"), "url": f"https://vk.com/video{oid}_{vid}"})
        elif t == "link":
            if obj.get("url"):
            ...
            out.append({"type": "audio", "artist": obj.get("artist"), "title": obj.get("title")})
    return out
...
def make_item(source_url, name, post, discovered_at):
    ...
    rp = repost_text(post)
    return {"id": f"{owner_id}_{post_id}", "community": name, "source_url": source_url, "post_url": url,
            "published_at": iso(post.get("date", 0)), "discovered_at": discovered_at, "text": text,
            "repost_text": rp, "attachments": simplify_attachments(post.get("attachments"))}
...
def visual_fingerprint(item, request):
    ...
    if not photo.is_absolute():
        photo = ROOT / photo
    inputs = {"id": str(item.get("id")), "template": request["template"], "headline": request["headline"],
              "detail": request["detail"], "photo_sha256": _file_sha256(photo),
              "archive_sha256": _file_sha256(VISUAL_ARCHIVE), "font_sha256": _file_sha256(VISUAL_FONT),
...
def build_feed_visuals(items, health, builder=None, output_dir=PUBLIC_VISUAL_DIR, fingerprint=visual_fingerprint):
    if builder is None:
    ...
    for item in items:
        ...
        try:
            ...
            if item.get("visual_fingerprint") == current_fingerprint and expected_output.is_file():
                ...
                reused += 1
                continue
            output = builder(item_id=item.get("id"), template=request["template"], original_photo=request["original_photo"],
                             headline=request["headline"], detail=request["detail"], output_dir=output_dir)
            output = Path(output)
...
def download_original_photo(session, source_url, target_path):
    headers = {"User-Agent": "Mozilla/5.0 (compatible; VKFeedMonitor/1.0)", "Referer": "https://vk.com/"}
...
def mirror_feed_photos(items, session, health):
    ...
    for item in items:
        ...
        if bool(text_key) and text_key in seen_texts:
            continue
        if text_key:
        ...
        for idx, attachment in enumerate(photos, start=1):
            ...
            try:
                ...
                errors.append({"post_url": item.get("post_url"), "photo_url": original_url, "error": str(e)[:300]})
...
def main():
    ...
    for line in SOURCES_FILE.read_text(encoding="utf-8").splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        sources.append((line, normalize_source(line)))
    ...
    health = {"checked_at": iso(), "api_version": API_VERSION, "source_count": len(sources), "ok": [], "errors": [],
              "new_posts": 0, "mirrored_photos": 0, "published_photos": 0, "image_errors": [],
              "built_visuals": 0, "built_visual_files": [], "reused_visuals": 0, "deleted_visuals": 0,
              "visual_errors": [], "note": "Ошибки здесь не содержат VK-токен. mirror_url ведёт на фото в GitHub Pages; release_url — резервная копия в GitHub Release."}
    ...
    for idx, (source_url, slug) in enumerate(sources, start=1):
        try:
            ...
            new_posts = [] if old_max is None else [p for p in posts if isinstance(p.get("id"), int) and p["id"] > old_max]
            ...
            if current_max is not None:
                state["sources"][slug] = {"source_url": source_url, "community": name,
                                          "max_post_id": max(current_max, old_max or 0), "last_ok_at": discovered_at}
            health["ok"].append({"source": source_url, "community": name, "latest_post_id": current_max, "fetched": len(posts)})
        except Exception as e:
    ...
    write_rss(kept)
    print(f"Источников: {len(sources)}; успешно: {len(health['ok'])}; ошибок: {len(health['errors'])}; новых постов: {health['new_posts']}; новых зеркал фото: {health['mirrored_photos']}; ошибок фото: {len(health['image_errors'])}")
